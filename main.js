...
function t0_shrink() {
    ...
    for (let i = 0; i < rows.length; i++) {
        if ((i+1 < start) || (i-2 > end)) {rows[i].classList.add("t0_hide")}
        else if (i-2 == end) {rows[i].cells[0].innerHTML = "..."}
    }
    rows[0].classList.remove("t0_hide")
}
function t0_highRow(i) {
    var rows = document.getElementById("t0_table").rows
    if(rows[i].classList.contains("t0_selTime")) {rows[i].classList.remove("t0_selTime")}
    else {rows[i].classList.add("t0_selTime")}
}

// Defunct


async function popAside(lstNo) {
    var table = document.getElementById("sidebar")
    var x = table.rows.length
    for (let i = 1; i < x; i++) {
        table.deleteRow(-1)
    }
    var sch = await readCSV('/CSV/sch.csv')
    var day = (new Date()).getDay() + 4
    /*for (let i=1; i < sch[2].length; i++) {
        if ((i % 2) == 1) {
            let date = sch[2][i]
    }*/
    if (sch[day].length >= 3) {
    for (let i=1; i < sch[day].length; i++) {
        if ((i % 3) == 1) {
            let row = table.insertRow(-1)
            let time = sch[day][i]
            let minutes = parseInt(time.substring(time.indexOf(':') + 1)) + parseInt(sch[1][parseInt(sch[day][i+2])*2])
            let hours = ((parseInt(time.substring(0, time.indexOf(':'))) + Math.floor(minutes/60))%24).toString()
            minutes = (minutes % 60).toString()
            if (minutes == "0") {minutes = "00"}
            let newtime = hours + ":" + minutes
            row.insertCell(0).outerHTML = "<th>" + time + " to " + newtime + " <br> " + sch[0][parseInt(sch[day][i+1])*2] + "</th>"
        }
    }}
}